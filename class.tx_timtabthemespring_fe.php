...
***************************************************************/
/**
 * class.tx_timtabthemespring_fe.php
 *
...
class tx_timtabthemespring_fe extends tslib_pibase {
	var $cObj; // The backReference to the mother cObj object set at call time
	// Default plugin variables:
	var $prefixId 		= 'tx_timtabthemespring_fe';		// Same as class name
	var $scriptRelPath 	= 'class.tx_timtabthemespring_fe.php';	// Path to this script relative to the extension dir.
	var $extKey 		= 'timtab_theme_spring';	// The extension key.
	...
	function substituteMarkers() {
		if($this->calledBy == 'tt_news') {	
			$this->markerArray['###SPRING_POST_ID###'] = $this->conf['data']['uid'];
			$this->markerArray['###SPRING_IN###'] = $this->pi_getLL('posted_in');
			$this->markerArray['###SPRING_BY###'] = sprintf($this->pi_getLL('by'), $this->conf['data']['author']);
			$this->markerArray['###SPRING_COMMENT_LINK###'] = $this->getCommentLink($this->conf['data']);		
		} elseif($this->calledBy == 've_guestbook') {		
			$this->markerArray['###SPRING_COMMENTS###'] = $this->pi_getLL('comments');
			$this->markerArray['###SPRING_SAID###'] = $this->pi_getLL('said');
		}
	}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/timtab_theme_spring/class.tx_timtabthemespring_fe.php']) {
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/timtab_theme_spring/class.tx_timtabthemespring_fe.php']);
}
